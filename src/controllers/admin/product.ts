...
export const updateProductAttributes = async (req: Request, res: Response) => {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const incomingAttributeIds = productAttributes
        .filter((attr: any) => attr.id)
        .map((attr: any) => attr.id);

      const attributesToDelete = existingAttributes.filter(
        (existing) => !incomingAttributeIds.includes(existing.id),
      );
      ...
      for (const incomingAttribute of productAttributes) {
        let attributeId = incomingAttribute.id;

        if (attributeId) {
          await tx.productAttribute.update({
            where: { id: attributeId },
            data: {
              name: incomingAttribute.name,
              type: incomingAttribute.type,
            },
          });
        } else {
        ...
        const existingAttribute = existingAttributes.find((attr) => attr.id === attributeId);
        const existingValues = existingAttribute?.values || [];

        const incomingValueIds = incomingAttribute.options
          .filter((option: any) => option.id)
          .map((option: any) => option.id);

        const valuesToDelete = existingValues.filter(
          (existingValue) => !incomingValueIds.includes(existingValue.id),
        );
        ...
        for (const option of incomingAttribute.options) {
          ...
          if (option.images && option.images.length > 0) {
            await tx.productAttributeValueImage.createMany({
              data: option.images.map((image: any, index: number) => ({
                productAttributeValueId: valueId,
                imageUrl: image.url,
                imageCldPubId: image.publicId,
                imageAltText: option.value,
                order: index,
                isPrimary: index === 0,
              })),
            });
    ...
    console.error(error);
    return res.status(500).json({ error: "Failed to update product attributes" });
  }
};

export const updateProductSkus = async (req: Request, res: Response) => {
  try {
    const productId = Number(req.params.id);
    ...
    return res.status(200).json({ message: "SKUs updated successfully" });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: "Failed to update SKUs" });
  }
};

export const toggleVariations = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { hasVariations } = req.body;
    ...
    console.error(error);
    res.status(500).json({ message: "Failed to toggle variations" });
  }
};
